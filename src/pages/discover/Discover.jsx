...
import { getMovies2 } from "../../redux/movie/movieAction";
import { GoDotFill } from "react-icons/go";

function  Discover({apiKey}) {
 ...
 useEffect(()=>{
  ...
  return (
    ...
    {movies.length === 0 ? (
        <p className="font-bold text-3xl">No movies found.</p>
    ) : (
        movies.map((movie) => (
            <div key={movie.id} className="movie-card  overflow-hidden">
                ...
                <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center">
                           <GoDotFill style={{color:'red'}} />112m
                        </div>
                    </div>
                </div>
            </div>
